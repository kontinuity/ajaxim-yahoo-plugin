...
class Ajax_IM {
    ...
    **/
   function logout() {
      //Do not login while trying to logout. That is just stupid
   ...
   function checkInfo($username, $password, $return=array()) {
     return yim_checkinfo($username, $password, $return);
   }
   ...
   function gc() {
      ...
      if(rand(1, 100) <= 30) {
         ...
         if($cleanup_event && mysql_num_rows($cleanup_event) > 0) {
         ...
         if($cleanup_event2 && mysql_num_rows($cleanup_event2) > 0) {
